...
constexpr int INTAKE_SPEED = -200;

pros::Controller master(pros::E_CONTROLLER_MASTER);

pros::adi::DigitalOut piston('H');                      // ADI port of Solenoid
pros::MotorGroup left_mg({4, 2, 3});                    // L motor smart ports
pros::MotorGroup right_mg({7, 5, 6});                   // R motor smart ports
...
void autonomous() {
        ...
        pros::delay(runtime);
        left_mg.move(leftmove * 0);
        right_mg.move(rightmove * 0);
}
...
void opcontrol() {
        ...
        while (true) {
                ...
                toggle = master.get_digital(DIGITAL_L1) && master.get_digital(DIGITAL_R1);

                //intake_mg.move(master.get_digital(DIGITAL_A) * INTAKE_SPEED);
                //intake_mg.move(master.get_digital(DIGITAL_A) * -127);
                intake_mg.move((master.get_digital(DIGITAL_L1) - master.get_digital(DIGITAL_R1)) * INTAKE_SPEED);
                //if (master.get_digital(DIGITAL_A) == HIGH){
                //        intake_mg.move(-100);
                //} else if (master.get_digital(DIGITAL_B) == HIGH) {
                //        intake_mg.move(100);
                //}

                get_controller_input(&leftmove, &rightmove);

                left_mg.move(master.get_analog(ANALOG_LEFT_Y) * -1);                                 // Sets left motor voltage
                right_mg.move(master.get_analog(ANALOG_RIGHT_Y));                               // Sets right motor voltage
